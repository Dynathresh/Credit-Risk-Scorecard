...
def main():
    ...
    # Converts continuous to WOE - only pass in feature_list_continuous
    ...
    # 系数list和截距
    coef_list, intercept, prediction_list, target_list = logistic_regression(train_data, app_train[target])
                # check_prediction(train_data.copy(), target, prediction_list, target_list)
...
def check_prediction(data, target, prediction_list, target_list):
...
def logistic_regression(train_data, target_column):
    X = train_data
    ...
    lr.fit(X_train, y_train)  # 把数据和目标加到模型里
    print(lr.coef_)
    print(lr.intercept_)
    coef_list = lr.coef_[0]  # 系数list
    intercept = lr.intercept_[0]  # 截距
    prediction_list = lr.predict(X_test)

    cm = confusion_matrix(y_test, prediction_list)
                # plt.figure()
                # plt.matshow(cm)
                # plt.title('Scorecard Confusion Matrix')
                # plt.colorbar()
    ...
    return coef_list, intercept, prediction_list, y_train

# Taken from scikit-learn docs: http://scikit-learn.org/stable/auto_examples/model_selection/plot_confusion_matrix.html
